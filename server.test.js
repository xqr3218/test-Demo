...
//     console.log('触发\'connection\'事件',2)
// });
...
//
// // 触发 connection 事件
// eventEmitter.removeListener('connection',connectHandler ,function(){
//     console.log('\'connectHandler\'事件被remove了me')
// });
// console.log(events.listeners)
//
// eventEmitter.emit('connection');
//
// console.log(events.prototype.on === events.prototype.addListener)  //true
// console.log("程序执行完毕。");
...
//
// var fs = require("fs");
// fs.readFile('./src/print.jsx', function (err, data) {
//     if (err){
...
var minSubArrayLen = function(s, nums) {
    ...
    while(sum >= s){
        ...
        console.log(sum, (Math.min(nums[i], nums[j])))
        console.log(len)
        if(sum < s){
            return len;
        }
        if(Math.min(nums[i], nums[j]) === nums[i]){
            i++;
        }
        else{
            j--;
